...
const vendor_upload = () => {
    ...
    const submitImg=async()=>{
      if(!imgForm.prompt1 || !form.title || !form.image ||!!imgForm.prompt2 || !imgForm.prompt3){
        return Alert.alert('Please fill in all the required data')
      }
      setUploading(true)
      try{
      ...
      <SafeAreaView className="bg-primary h-full">
        ...
        <View className="flex-row mt-5 mb-7 justify-around">
          <TouchableOpacity onPress={() => setSelectedTab('video')}>
            <Text className={`text-lg ${selectedTab === 'video' ? 'text-white' : 'text-gray-400'}`}>Upload Video</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setSelectedTab('image')}>
            <Text className={`text-lg ${selectedTab === 'image' ? 'text-white' : 'text-gray-400'}`}>Upload Image</Text>
          </TouchableOpacity>
        </View>
        {selectedTab === 'video' ? (
          <><FormField 
          ...
          <View className="mt-7 space-y-2">
            ...
            <TouchableOpacity onPress={()=>openPicker('video',selectedTab)}>
             {form.video?(<Video
          ...
          <TouchableOpacity  onPress={()=>openPicker('image',selectedTab)}>
        ...
        /></>
          ...
          <TouchableOpacity  onPress={()=>openPicker('image',selectedTab)}>
             {imgForm.image?(<Image source={{uri:imgForm.image.uri}}
             className="w-full h-64 rounded-2xl"
             resizeMode='cover'
             />):(
              <View className="w-full h-40 px-4 bg-black-100 rounded-2xl justify-center items-center">
